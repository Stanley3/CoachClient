...
import com.chejiawang.android.cache.CacheManager;
import com.chejiawang.android.ui.DividerItemDecoration;
...
import com.chejiawang.android.utils.WeakAsyncTask;
...
public class EvaluateBaseRecycleViewFragment extends BasedFragment implements
		RecycleBaseAdapter.OnItemClickListener,
		RecycleBaseAdapter.OnItemLongClickListener {
	public static final String BUNDLE_KEY_CATALOG = "BUNDLE_KEY_CATALOG";
	private static final String TAG = "EvaluateBaseRecycleViewFragment";
	...
	static class ReadCacheTask extends
			WeakAsyncTask<Void, Void, byte[], EvaluateBaseRecycleViewFragment> {

		public ReadCacheTask(EvaluateBaseRecycleViewFragment target) {
			super(target);
		}

		@Override
		protected byte[] doInBackground(EvaluateBaseRecycleViewFragment target,
				Void... params) {
			if (target == null) {
				return null;
			}
		...
		@Override
		protected void onPostExecute(EvaluateBaseRecycleViewFragment target,
				byte[] result) {
	...
	private static class ParserTask extends AsyncTask<Void, Void, String> {
		private WeakReference<EvaluateBaseRecycleViewFragment> mInstance;
		...
		public ParserTask(EvaluateBaseRecycleViewFragment instance,
				byte[] responseData, boolean fromCache) {
			super();
			this.mInstance = new WeakReference<EvaluateBaseRecycleViewFragment>(
					instance);
		...
		protected String doInBackground(Void... params) {
			EvaluateBaseRecycleViewFragment instance = mInstance.get();
			if (instance == null)
				return null;
			try {
				...
				list = data.getList();
				Logger.e(TAG, "get the list:" + list.toString());
				map = data.getMap();

			} catch (Exception e) {
				Logger.e(TAG, "get the list error");
				e.printStackTrace();
				parseError = true;
			}
			return null;
		...
		protected void onPostExecute(String result) {
			...
			if (instance != null) {
				if (parseError) {
					...
					instance.executeOnLoadDataSuccess(list, map);
					if (!fromCache) {
	...
	protected void executeOnLoadDataSuccess(List<?> data, Map<?, ?> map) {
		if (mState == STATE_REFRESH) {
			mAdapter.clear();
		}
		Logger.e(TAG, "catch data:" + data.toString());
		mAdapter.addData(data);
		if (map != null) {
			Intent intent = new Intent();
			intent.setAction(Contants.EvalutionBroadCast);
			Bundle bundle = new Bundle();
			bundle.putString("total", map.get("total").toString());
			bundle.putString("goodEvaluation", map.get("goodEvaluation")
					.toString());
			bundle.putString("midEvaluation", map.get("midEvaluation")
					.toString());
			bundle.putString("badEvaluation", map.get("badEvaluation")
					.toString());
	...
	private static class ResponseHandler extends AsyncHttpResponseHandler {
		...
		public ResponseHandler(EvaluateBaseRecycleViewFragment instance) {
			mInstance = new WeakReference<EvaluateBaseRecycleViewFragment>(
					instance);
		}
